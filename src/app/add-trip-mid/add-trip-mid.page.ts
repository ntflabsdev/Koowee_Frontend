...
@Component({
  ...
  templateUrl: './add-trip-mid.page.html',
  styleUrls: ['./add-trip-mid.page.scss'],
})
export class AddTripMidPage implements OnInit {
  location: any;
  ...
  submit(status) {
    ...
    this.userService.postData(frmDatas, config.API_URL + '/upload_img_trip').subscribe((result) => {
      ...
      this.userService.postData(datas, config.API_URL + '/add_trip_single').subscribe((result) => {
